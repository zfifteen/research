...
The Padovan Principle: Gap Structure and Error Propagation in Linear Recurrences
Version 3 — Causal Isolation, Noise Independence, Architectural Prediction

NOTE: This work does not claim Padovan is superior for any application.
The claim is specifically: gap structure is a causal variable in error
propagation behavior, and the mechanism is identifiable in the eigenvalue
spectrum of the companion matrix.

Contributions:
  - Gap topology sweep: demonstrates gap structure affects error propagation
  - Three noise regimes: Gaussian, correlated drift, systematic bias
  - Rigorous stats: KS tests, bootstrap CIs, effect size
  - Architectural simulation: synthetic dimensional deviation study
  - Application sketches: RMSG, GIGA, PNSF

Author: Fate + Claude (March 2026)
"""
...
SAVE_DIR  = "plastic_whitepaper_assets_v4"
...
RECURRENCES = {
    "Padovan [2,3]"         : [2, 3],   # subject of interest
...
def figure_gap_topology():
    ...
    plt.suptitle('Figure 1: Gap Structure Affects Error Propagation — Three Noise Regimes',
    ...
    # Print KS-test table
    print("\n  KS-test: Padovan vs each other recurrence (Gaussian noise)")
...
def figure_noise_regimes():
    ...
    Documents how error propagation behavior differs by gap structure
    ...
    plt.suptitle('Figure 2: Error Propagation Behavior Differs by Gap Structure Across Noise Regimes',
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def figure_eigenvalue_anatomy():
    ...
    damping factor to error performance.
    """
    ...
    ax2.set_yscale('log')
    ax2.set_title('Damping Factor Correlates With Error Behavior\n(Eigenvalue Structure → Empirical link)',
                  fontweight='bold')
    ...
    plt.suptitle('Figure 3: Eigenvalue Damping Correlates With Error Behavior (Identified Mechanism)',
                 fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def figure_architectural_simulation():
    ...
    fig.suptitle(
        ...
        f"Dimensional Deviation Distributions Differ by Recurrence Structure\n"
...
def demo_applications():
    ...
    print("\n1. RMSG — Modular Scaling Generator (gap-based)")
    print("   Scale series generated via Padovan recurrence under noisy conditions")

    def rmsg(n_scales, base=1.0, noise_std=0.02):
        """
        Scale series generated using Padovan recurrence with correlated noise.
        Demonstrates ratio behavior under noisy conditions.
        Warm-started from ideal Padovan values to avoid transient artifacts.
        """
    ...
    print("\n3. PNSF — Gapped Noise-Shaping Filter")
    print("   Quantization error feedback using gap delays — SNR behavior vs standard shaping")
    ...
    print(f"   PNSF vs naive delta:                   {snr_pnsf - snr_naive:+.2f} dB")
    print(f"   PNSF vs standard shaping delta:        {snr_pnsf - snr_std:+.2f} dB")

    return {
        'rmsg_max_deviation': float(max(abs(r-RHO) for r in ratios)),
        'pnsf_snr_improvement_dB': float(snr_pnsf - snr_naive),
    }
...
WHITEPAPER_v4 = """
================================================================================
GAP STRUCTURE AS A CAUSAL VARIABLE IN ERROR PROPAGATION
OF GAPPED LINEAR RECURRENCES (v4)
================================================================================
Date: {date}

Abstract
--------
We demonstrate that gap structure — specifically, whether a linear recurrence
samples its immediate predecessor — is a causal variable in how implementation
errors propagate through a sequence. Using the Padovan recurrence P(n) = P(n-2)
+ P(n-3) as the primary example, we show via companion matrix analysis that
skipping the n-1 term splits error propagation into two parallel paths with
a phase relationship determined by the damped complex eigenvalues. This produces
measurably distinct error behavior compared to contiguous recurrences.

We make no claim that Padovan is superior for any particular application. The
claim is that the phenomenon is real, the mechanism is identifiable, and the
behavior is predictable from the eigenvalue structure alone.

1. Causal Isolation
--------------------
The key question: is error propagation behavior determined by the growth rate
(dominant eigenvalue), or by the gap structure (topology of the dependency graph)?

We sweep six recurrence topologies holding all noise parameters fixed:
Padovan [2,3], Fibonacci [1,2], wide-gap [2,4], triple-path [3,5,7],
adjacent-gap [1,3], and pure-multiplicative [1,1].

If growth rate were the causal variable, recurrences with similar dominant
eigenvalues should produce similar error distributions. The data does not
support this. If gap structure is causal, error behavior should correlate with
the second-largest eigenvalue magnitude (the damping factor). It does
(Pearson r reported in Figure 3).

2. Noise-Model Independence
----------------------------
Three noise models confirm the mechanism is not noise-shape-specific:
  - Gaussian IID: independent per-step scatter
  - AR(1) correlated: craftsman who repeats the same mistake (ρ_AR = 0.7)
  - Systematic bias: tool that consistently cuts short (+2% bias)

The distinct error propagation behavior of gapped recurrences persists across
all three regimes. The gap structure is operating on a property that all three
noise types share: sequential compounding through the dependency chain.

3. Architectural Prediction
-----------------------------
We simulate synthetic dimensional measurements from Padovan-designed vs.
Fibonacci-designed structures under systematic construction noise. The resulting
deviation distributions are statistically distinguishable (KS test, Levene test,
Cohen's d reported in Figure 4).

Testable empirical prediction: photogrammetric or laser surveys of van der
Laan's Abbey of St. Benedictusberg, compared against contemporary golden-ratio
structures of similar era and construction method, should show measurably
distinct dimensional deviation distributions. No directional claim is made
about which system will show lower absolute error — only that the distributions
should differ in a way consistent with the recurrence topology.
...
Error injection at step k:
  ...
  The damped modes carry the error; the dominant mode carries the signal.
  Net behavior: error does not compound coherently.

5. Application Sketches (Hypotheses Only)
-------------------------------------------
The following are research directions suggested by the mechanism. None are
validated claims.

RMSG (Modular Scaling Generator):
  Scale series generated via Padovan recurrence. Ratio deviation from ρ
  observable under noisy conditions. Whether this is useful depends entirely
  on the application's noise profile and convergence requirements.

GIGA (Gapped Inheritance Genetic Algorithm):
  Crossover operator using gap offsets [2,3]. Hypothesis: error compounding
  during fitness landscape traversal may differ from contiguous crossover.
  Untested.

PNSF (Gapped Noise-Shaping Filter):
  Quantization error feedback using gap delays. SNR behavior differs from
  standard 1st-order shaping. Whether the difference is useful is application-
  dependent.

6. Conclusion
--------------
The gap in P(n) = P(n-2) + P(n-3) produces a specific, identifiable, and
measurable effect on error propagation behavior. The mechanism is the zero
in position (1,1) of the companion matrix — it splits error into two paths
with a phase relationship determined by the damped complex eigenvalues.

This phenomenon was present in van der Laan's architectural system for decades
without being formally identified. It is now named, mechanistically explained,
empirically demonstrated across three noise regimes, and expressed as a
falsifiable architectural prediction.

The gap is the feature. What you do with it is a separate question.
================================================================================
"""
...
def main():
    print(WHITEPAPER_v4.format(date=datetime.now().strftime('%B %d, %Y')))
