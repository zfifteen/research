...
# -*- coding: utf-8 -*-
"""
Manufactured-solution gradient reconstruction harness for setup_3.

Runs GG, WLSQ, and GLSQ (alpha in {0.0, 0.5, 1.0}) across discovered meshes,
produces comparison tables, plots, CSVs, and a markdown validation report with
revised-hypothesis (P1-P6) assessment.
"""
...
def _close_rel(a: float, b: float, rtol: float = 1.0e-10, atol: float = 1.0e-14) -> bool:
    return abs(a - b) <= (atol + rtol * max(abs(a), abs(b)))


def _evaluate_revised_hypothesis(summary_rows: list[dict[str, Any]], metrics: list[dict[str, Any]]) -> dict[str, Any]:
    checks: dict[str, dict[str, str]] = {}

    def add_check(pid: str, state: str, details: str) -> None:
        checks[pid] = {"state": state, "details": details}

    # P1: Controlled dominance WLSQ < GLSQ(0.5) < GG.
    if not summary_rows:
        add_check("P1", "not_evaluable", "No controlled summary rows found.")
    else:
        missing = []
        failed = []
        for row in summary_rows:
            w = float(row["pure_lsq_error"])
            g = float(row["pure_gg_error"])
            h = float(row["glsq_05_error"])
            if not (math.isfinite(w) and math.isfinite(g) and math.isfinite(h)):
                missing.append(row["mesh_name"])
                continue
            if not (w < h < g):
                failed.append(f"{row['mesh_name']} (WLSQ={w:.3e}, GLSQ={h:.3e}, GG={g:.3e})")
        if missing:
            add_check("P1", "not_evaluable", f"GLSQ(α=0.5) metrics missing for: {', '.join(missing)}")
        elif failed:
            add_check("P1", "fail", "Ordering violated for: " + "; ".join(failed))
        else:
            add_check("P1", "pass", "Ordering holds for all controlled meshes.")

    # P2: Mixed dominance (optional).
    mixed_rows = [m for m in metrics if m.get("mesh_type") == "mixed"]
    if not mixed_rows:
        add_check("P2", "not_applicable", "Mixed mesh not provided in this run.")
    else:
        gg = _safe_get_error(mixed_rows, "gg")
        ww = _safe_get_error(mixed_rows, "wlsq")
        hh = _safe_get_error(mixed_rows, "glsq", 0.5)
        if gg is None or ww is None or hh is None:
            add_check("P2", "not_evaluable", "Missing one or more required mixed-mesh metrics (GG/WLSQ/GLSQ α=0.5).")
        elif ww < hh < gg:
            add_check("P2", "pass", f"Mixed ordering holds (WLSQ={ww:.3e}, GLSQ={hh:.3e}, GG={gg:.3e}).")
        else:
            add_check("P2", "fail", f"Mixed ordering violated (WLSQ={ww:.3e}, GLSQ={hh:.3e}, GG={gg:.3e}).")

    # P3: Bounded hybrid advantage.
    adv_rows = [r for r in summary_rows if math.isfinite(float(r["hybrid_advantage"]))]
    if not adv_rows:
        add_check("P3", "not_evaluable", "Hybrid-advantage data unavailable (GLSQ α=0.5 missing).")
    else:
        adv = np.array([float(r["hybrid_advantage"]) for r in adv_rows], dtype=float)
        if np.any(adv <= 1.0) or np.any(adv >= 3.0):
            add_check("P3", "fail", f"Hybrid-advantage outside revised bound (min={adv.min():.3f}, max={adv.max():.3f}).")
        elif float(np.max(adv)) >= 5.0:
            add_check("P3", "fail", f"Hybrid-advantage reaches old strong-peak scale (max={np.max(adv):.3f}).")
        else:
            add_check("P3", "pass", f"Hybrid-advantage remains bounded (min={adv.min():.3f}, max={adv.max():.3f}).")

    # P4: No robust mean-z transition-window peak rule.
    if not adv_rows:
        add_check("P4", "not_evaluable", "Peak hybrid-advantage cannot be evaluated without GLSQ α=0.5.")
    else:
        peak = max(adv_rows, key=lambda r: float(r["hybrid_advantage"]))
        peak_adv = float(peak["hybrid_advantage"])
        peak_z = float(peak["z_mean"])
        if 3.0 <= peak_z <= 6.0:
            add_check("P4", "fail", f"Peak advantage {peak_adv:.3f}x occurs inside mean-z transition band (z={peak_z:.2f}).")
        else:
            add_check("P4", "pass", f"Peak advantage {peak_adv:.3f}x occurs outside mean-z transition band (z={peak_z:.2f}).")

    # P5: Endpoint identities for L2 and |C|max.
    by_key: dict[tuple[str, str], list[dict[str, Any]]] = {}
    for row in metrics:
        by_key.setdefault((row.get("mesh_type", "controlled"), row["mesh_name"]), []).append(row)

    endpoint_missing = []
    endpoint_failed = []
    for (mesh_type, mesh_name), rows in by_key.items():
        ww = next((r for r in rows if r["scheme"] == "wlsq"), None)
        gg = next((r for r in rows if r["scheme"] == "gg"), None)
        g0 = next((r for r in rows if r["scheme"] == "glsq" and abs(float(r["alpha"]) - 0.0) <= 1.0e-12), None)
        g1 = next((r for r in rows if r["scheme"] == "glsq" and abs(float(r["alpha"]) - 1.0) <= 1.0e-12), None)
        if ww is None or gg is None or g0 is None or g1 is None:
            endpoint_missing.append(f"{mesh_type}:{mesh_name}")
            continue

        same_l2_g0 = _close_rel(float(g0["l2_error"]), float(ww["l2_error"]))
        same_l2_g1 = _close_rel(float(g1["l2_error"]), float(gg["l2_error"]))
        same_c_g0 = _close_rel(float(g0["cmax"]), float(ww["cmax"]))
        same_c_g1 = _close_rel(float(g1["cmax"]), float(gg["cmax"]))
        if not (same_l2_g0 and same_l2_g1 and same_c_g0 and same_c_g1):
            endpoint_failed.append(f"{mesh_type}:{mesh_name}")

    if endpoint_missing:
        add_check("P5", "not_evaluable", "Missing endpoint metrics for: " + ", ".join(endpoint_missing))
    elif endpoint_failed:
        add_check("P5", "fail", "Endpoint identities failed for: " + ", ".join(endpoint_failed))
    else:
        add_check("P5", "pass", "Endpoint identities hold for all evaluated meshes.")

    # P6: Runtime ordering GG <= WLSQ <= GLSQ(0.5) with <=10% tolerance.
    runtime_total = 0
    runtime_pass = 0
    runtime_missing = []
    for (mesh_type, mesh_name), rows in by_key.items():
        t_gg = _safe_get_time(rows, "gg")
        t_w = _safe_get_time(rows, "wlsq")
        t_h = _safe_get_time(rows, "glsq", 0.5)
        if t_gg is None or t_w is None or t_h is None:
            runtime_missing.append(f"{mesh_type}:{mesh_name}")
            continue
        runtime_total += 1
        ok = (t_gg <= 1.10 * t_w) and (t_w <= 1.10 * t_h)
        if ok:
            runtime_pass += 1

    if runtime_total == 0:
        add_check("P6", "not_evaluable", "Missing runtime metrics for GG/WLSQ/GLSQ α=0.5.")
    else:
        ratio = runtime_pass / runtime_total
        if ratio >= 0.8:
            add_check("P6", "pass", f"Runtime ordering holds for {runtime_pass}/{runtime_total} meshes ({ratio:.0%}).")
        else:
            add_check("P6", "fail", f"Runtime ordering holds for {runtime_pass}/{runtime_total} meshes ({ratio:.0%}), below 80%.")

    required = ["P1", "P3", "P4", "P5", "P6"]
    if checks["P2"]["state"] not in ("not_applicable",):
        required.append("P2")
    req_states = [checks[k]["state"] for k in required]

    if any(s == "not_evaluable" for s in req_states):
        overall_status = "inconclusive"
        failed_or_missing = [k for k in required if checks[k]["state"] in ("not_evaluable", "fail")]
        sentence = "Revised hypothesis is inconclusive because required checks are incomplete or failed: " + ", ".join(failed_or_missing) + "."
    elif any(s == "fail" for s in req_states):
        overall_status = "falsified"
        failed = [k for k in required if checks[k]["state"] == "fail"]
        sentence = "Revised hypothesis is falsified because these checks failed: " + ", ".join(failed) + "."
    else:
        overall_status = "validated"
        sentence = "Revised hypothesis is validated because all required checks passed."

    return {"status": overall_status, "sentence": sentence, "checks": checks}
...
def _write_markdown_report(
    ...
    assessment = _evaluate_revised_hypothesis(summary_rows, metrics)
    status = assessment["status"]
    hypothesis_sentence = assessment["sentence"]
    ...
    lines.append(f"- Revised-hypothesis status: **{status.upper()}**")
    lines.append("")
    ...
    lines.append("## Revised-Hypothesis Assessment")
    lines.append("")
    lines.append(f"- {hypothesis_sentence}")
    lines.append("")
    lines.append("| Prediction | Outcome | Details |")
    lines.append("|---|---|---|")
    for pid in ("P1", "P2", "P3", "P4", "P5", "P6"):
        check = assessment["checks"][pid]
        lines.append(f"| {pid} | {check['state'].upper()} | {check['details']} |")
    lines.append("")
...
def run_full_validation() -> Path:
    parser = argparse.ArgumentParser(description="Manufactured-solution gradient validation harness (revised hypothesis).")
